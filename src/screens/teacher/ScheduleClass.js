...
import { Timestamp } from '../../services/dataShim';
...
const ScheduleClass = ({ navigation, route }) => {
  ...
  const [dateStr, setDateStr] = useState(
    existing?.scheduledAt
      ? `${pad(existing.scheduledAt.toDate().getDate())}/${pad(
          existing.scheduledAt.toDate().getMonth() + 1,
        )}/${existing.scheduledAt.toDate().getFullYear()}`
      : `${pad(initialDate.getDate())}/${pad(
          initialDate.getMonth() + 1,
        )}/${initialDate.getFullYear()}`,
  );
  const [timeStr, setTimeStr] = useState(
    existing?.scheduledAt
      ? `${pad(existing.scheduledAt.toDate().getHours())}:${pad(
          existing.scheduledAt.toDate().getMinutes(),
        )}`
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      */
      Toast.success(
        'Students enrolled in this batch have been notified.',
        'Class scheduled',
      );
      navigation.goBack();
    } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        title={existing ? 'Edit Class' : 'Schedule Class'}
        showBack
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {batches.length === 0 ? (
          <Text style={styles.notice}>
            You have no batches yet. Ask the admin to assign you a batch.
          </Text>
        ) : (
          batches.map(batch => (
            <TouchableOpacity
            ...
            >
              <View
                style={[
                  styles.batchColorDot,
                  { backgroundColor: batch.color || colors.primary },
                ]}
              />
              <View style={{ flex: 1 }}>
        ...
        <View style={styles.durationRow}>
          {DURATIONS.map(d => (
            <TouchableOpacity
              ...
              style={[
                styles.durationBtn,
                durationMin === d && styles.durationBtnActive,
              ]}
        ...
        <Button
          label={existing ? 'Update Class' : 'Schedule Class'}
